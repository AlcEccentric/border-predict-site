...
const getSections = (isAnniversary: boolean): Section[] => [
    {
        ...
        content: isAnniversary ? (
            ...
            <>
                <p>
                    予測値は過去の類似イベントのデータをもとに算出していますが、実際の結果とは異なる場合があります。参考情報としてご利用ください。
                </p>
                <p className="mt-2">
                    <strong>予測の流れ:</strong><br />
                    1. 現在のイベントデータを正規化し、過去のイベントと比較して近傍イベントを選定<br />
                    2. 近傍イベントのスコア推移を現在の傾向に合わせて調整<br />
                    3. 類似度に応じて重み付けを行い、複数のイベントから予測値を算出
                </p>
    ...
    ...(isAnniversary ? [{
        ...
        content: (
            <>
                <p>
                    上のグラフとスコア表示にある「安全ライン」は、そのスコアに到達した時点で、最終的に安全圏に入る確率が一定以上になると見込まれる目標ラインです。
                </p>
                <p className="mt-2">
                ...
                <div className="mt-4">
                    ...
                    <p className="text-sm mb-2">
                        70%・80%・90%の3段階から選択できます。数字が大きいほど安全圏に入る確率は高くなりますが、その分、目標となるスコアも高くなります。
                    </p>
                    <p className="mt-2 text-sm text-base-content/70">
                        → イベント序盤は今後の変動が大きいため、予測値と安全ラインとの差も大きくなります。イベントが進むにつれて予測の不確実性が小さくなり、安全ラインも予測値に近づいていきます。
                    </p>
                </div>
                <div className="mt-4">
                    <h4 className="text-lg font-semibold mb-2">算出の流れ</h4>
                ...
                <p className="mt-4 text-sm">
                    ...
                    参加者の行動やイベント固有の要因などによって、実際の結果が安全ラインを上回ることもあります。あらかじめご了承ください。
                </p>
            </>
        ),
    }]),
    ...
    ...(isAnniversary ? [{
        ...
        content: (
            <p className="text-sm">
                今回のイベントでは、累積報酬で期間限定のスキップパスを獲得できるほか、DX+パス購入者はオートパスを無制限に追加購入できるようになりました。

                現時点では、100位ボーダーは従来より約3割高いペースで推移していることを確認しています。
                これが序盤特有の一時的な動きなのか、それともイベント全体を通じた傾向なのかは、現時点ではまだ判断できません。

                今後数日間の推移を注視し、必要に応じて予測モデルの調整を行う予定です。
                
                そのため、予告なく予測結果が変動する場合がありますので、あらかじめご了承ください。
            </p>
        ),
    }] : []),
    {
