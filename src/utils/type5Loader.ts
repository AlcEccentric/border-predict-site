...
/**
 * Demo-mode helper: synthesize Type-5-shape final-only CI bounds (scalar
 * upper/lower) on top of an existing prediction. Only used by demo mode
 * where R2 hosts an older data layout without bounds; production data
 * ships real bounds and this is bypassed.
 *
 * The fakery is a fixed envelope around the final score. Don't use this
 * to draw any inferences — it's purely so we can see the new CI rows
 * render against historical fixture data.
 */
export function injectFakeBounds(idol: IdolPredictionData): IdolPredictionData {
    const fakeOne = (p: PredictionData | null): PredictionData | null => {
        if (!p) return null;
        if (p.data.raw.bounds) return p; // already has real bounds
        const target = p.data.raw.target;
        const final = target[target.length - 1];
        if (typeof final !== 'number') return p;
        return {
            ...p,
            data: {
                ...p.data,
                raw: {
                    ...p.data.raw,
                    bounds: {
                        75: { lower: final * 0.92, upper: final * 1.08 },
                        90: { lower: final * 0.86, upper: final * 1.14 },
                    },
                },
            },
        };
    ...
    return {
        ...idol,
        prediction100: fakeOne(idol.prediction100),
        prediction1000: fakeOne(idol.prediction1000),
    };
}
